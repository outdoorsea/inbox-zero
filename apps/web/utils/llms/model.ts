...
function selectModel(
  ...
  switch (aiProvider) {
    ...
    case Provider.OLLAMA: {
      ...
      if (!env.OLLAMA_BASE_URL) throw new Error("Ollama base URL is not set");
      return {
...
function selectEconomyModel(userAi: UserAIFields) {
  if (env.ECONOMY_LLM_PROVIDER && env.ECONOMY_LLM_MODEL) {
    // Special handling for Ollama which doesn't require a traditional API key
    if (env.ECONOMY_LLM_PROVIDER === Provider.OLLAMA) {
      return selectModel({
        aiProvider: env.ECONOMY_LLM_PROVIDER,
        aiModel: env.ECONOMY_LLM_MODEL,
        aiApiKey: "ollama", // Placeholder for Ollama
      });
    }
...
function selectDefaultModel(userAi: UserAIFields) {
  ...
  // Special handling for Ollama which doesn't need a traditional API key
  if (aiProvider === Provider.OLLAMA) {
    aiModel = aiModel || env.NEXT_PUBLIC_OLLAMA_MODEL;
  }
...
function getProviderApiKey(
  ...
  const providerApiKeys = {
    ...
    [Provider.OLLAMA]: "ollama", // Ollama doesn't need a traditional API key
  };
